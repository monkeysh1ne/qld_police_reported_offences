...
# offences in Queensland between 2001 - 2023.
...
# Police Division Reported Offences Number [https://open-crime-data.s3-ap-southeast-2.amazonaws.com/Crime%20Statistics/division_Reported_Offences_Number.csv]
...
small_df <- working %>% select(year, month, Division, Robbery) #%>% 
  # filter(year == 2022)


# aggregate data by Division and year
working_agg1 <- aggregate(Robbery ~ Division + year,
                          small_df,
                          FUN = sum)
